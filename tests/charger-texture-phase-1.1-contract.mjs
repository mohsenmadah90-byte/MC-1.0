...
for(const file of ["mcity_charger_powered.particle.json","mcity_charger_charging.particle.json"]) { const data=JSON.parse(fs.readFileSync(path.join(root,"resource_packs/mcity_resources/particles",file))); assert.ok(data.particle_effect?.description?.identifier); }

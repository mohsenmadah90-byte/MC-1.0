...
function blank(id,name){return{playerId:clean(id),playerName:clean(name,64),friends:{},incoming:{},outgoing:{},messages:[],updatedAt:now()};}
export class FriendService{
 ...
 static #messageTimes=new Map();
 static sendMessage(player,targetId,body){const id=clean(targetId),text=clean(body,500);if(!text)return{success:false,message:"§cMessage cannot be empty."};if(!this.isFriend(player.id,id))return{success:false,message:"§cYou can message friends only."};const key=`${player.id}:${id}`,last=this.#messageTimes.get(key)||0;if(now()-last<1500)return{success:false,message:"§cPlease wait before sending another message."};this.#messageTimes.set(key,now());const message={id:`msg_${now()}_${Math.floor(Math.random()*1000000)}`,senderId:player.id,senderName:clean(player.name,64),body:text,createdAt:now(),read:false};const tx=Database.transaction(COLLECTION,data=>{const receiver=this.#get(data,id);receiver.messages=(receiver.messages||[]).slice(-199);receiver.messages.push(message);receiver.updatedAt=now();return message;});if(tx.success){try{NotificationService.create(id,{type:"friend_message",source:"friends",title:"New Message",message:`You have a new message from ${player.name}.`,action:"friends"});}catch{}}return tx.success?{success:true,message:"§aMessage sent."}:{success:false,message:`§c${tx.error}`};}
 static messages(player){return [...(this.db().players[player.id]?.messages||[])].sort((a,b)=>(b.createdAt||0)-(a.createdAt||0));}
 static markMessageRead(player,messageId){const tx=Database.transaction(COLLECTION,data=>{const p=this.#get(data,player.id);const m=(p.messages||[]).find(x=>x.id===messageId);if(!m)return false;m.read=true;p.updatedAt=now();return true;});return tx.success&&!!tx.result;}
